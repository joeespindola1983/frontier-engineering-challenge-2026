...
The first controlled model comparison is complete. On the two implemented cases, the bounded WAKE agent scored 63.34/100 versus 38.86/100 for the direct-call baseline using frozen grader v1.1. The primary metric, first schema versions, 16-case registry, two public fixtures, permanent TDD policy, compact baseline bundle, comparable prompts, Python/uv evaluation runtime, GPT-5.6 Terra medium configuration, four deterministic investigation tools, bounded single-agent loop, verifier, observable trajectory format, monotonic case/run runtime instrumentation, coach-facing React/Vinext interface, and local task-level product service are accepted. The paid single-case preflight remains calibration-only. Replay is the default; live product execution requires explicit server and client opt-in plus `OPENAI_API_KEY`. Checkpoint and memory state remain in process; no upload parser API, database, authentication, or durable memory has been accepted yet. Record new stable choices in `docs/DECISIONS.md`.
