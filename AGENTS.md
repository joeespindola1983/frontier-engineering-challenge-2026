...
The expanded controlled model comparison is complete. Across ten implemented v2 cases, the bounded WAKE agent scored 83.76/100 versus 49.00/100 for the direct-call baseline using grader v1.2: +34.76 points at US$0.283344 incremental cost. Every case improved, but environmental interpretation regressed from 80% to 76%, and the single real anonymized case remains WAKE's weakest at 53.71. The historical two-case v1 registry, grader, outputs, and 63.34-versus-38.86 result remain unchanged. The progressive-evidence v1 ablation is also preserved unchanged: core plan + SpeedCoach passed 7/8 applicable checks but added an unsupported derived-distance deviation; context/environment passed 10/10; full mobile evidence passed 12/12; cross-condition consistency failed at US$0.298604. The versioned v2 workflow marks reconstructed segment distance as insufficient for prescribed-distance completion, reinforces that boundary in the prompt, and rejects conflicting deviations in the verifier. Its official repeat passed core 8/8, context/environment 10/10, full 12/12, and cross-condition consistency at US$0.358676. The two owner-authorized demo-club candidate investigations are also complete and preserved: Bridge isolated `work-02` at 18 SPM, Atlas isolated `recovery-02` at 247 seconds, and both passed deterministic output verification for US$0.194118 combined and 60,094 tokens. The reproducible two-week public batch now contains 40 isolated real-informed synthetic activity records: 40 data-validated, 38 water sessions reconstructed, 37 plan-compared, two agent-verified, and zero human-approved. Its batch API prepares up to 100 independent source groups, isolates failure, resumes after restart, executes sequentially through explicit start gates, and never combines sessions into one model prompt. Two Concept2-shaped indoor records deliberately remain `SOURCE_ADAPTER_REQUIRED`. Longitudinal club synthesis remains unauthorized and `NOT_EXECUTED`. The primary metric, first schema versions, versioned 16-case registries, public fixtures and verifiers, permanent TDD policy, versioned compact baseline bundles, comparable prompts, Python/uv evaluation runtime, GPT-5.6 Terra medium configuration, four deterministic investigation tools, bounded single-agent loop, verifier, observable trajectory format, monotonic case/run runtime instrumentation, coach-facing React/Vinext interface, local task-level product service, deterministic SpeedCoach/WAKE-mobile normalization, progressive plan+SpeedCoach compact-summary preparation with optional mobile/environment/context enhancers, explicit prepared-bundle and batch execution, generic analysis-to-review/checkpoint/briefing/memory transitions, per-execution cost authorization, a restart-safe local session inbox, three frozen ground-truth-free evidence-ablation inputs, an all-condition runner, a condition-aware capability reporter, and a submission-only read-only Evaluation view are accepted. The Evaluation view is generated from committed public aggregates, contains no ground truth or raw evidence, and cannot call the model. Replay is the default; all live product execution requires explicit mode, positive finite cost authorization, and `OPENAI_API_KEY`. The authorization is a start gate, not a provider cap. Only the exact five-source public case-002 upload bundle can reuse its product replay; all saved evaluation outputs can be reviewed and regraded offline without another model call. The results do not establish human-coach superiority, broad generalization, improved athletic performance, or durable billing control. The local state file is Git-ignored and user-restricted but not encrypted, authenticated, multi-tenant, backed up, or a production database. Record new stable choices in `docs/DECISIONS.md`.
