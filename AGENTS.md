...
The first controlled model comparison is complete. On the two implemented cases, the bounded WAKE agent scored 63.34/100 versus 38.86/100 for the direct-call baseline using frozen grader v1.1. The primary metric, first schema versions, 16-case registry, two public fixtures, permanent TDD policy, compact baseline bundle, comparable prompts, Python/uv evaluation runtime, GPT-5.6 Terra medium configuration, four deterministic investigation tools, bounded single-agent loop, verifier, observable trajectory format, monotonic case/run runtime instrumentation, coach-facing React/Vinext interface, local task-level product service, process-local typed source intake, and deterministic SpeedCoach/WAKE-mobile normalization are accepted. The paid single-case preflight remains calibration-only. Replay is the default; live product execution requires explicit server and client opt-in plus `OPENAI_API_KEY`. Only the exact public case-002 upload bundle can reuse its replay. New-bundle compact-summary assembly, checkpoint and memory persistence, database, authentication, club tenancy, and durable uploads remain unimplemented. Record new stable choices in `docs/DECISIONS.md`.
