...
The first controlled model comparison is complete. On the two implemented cases, the bounded WAKE agent scored 63.34/100 versus 38.86/100 for the direct-call baseline using frozen grader v1.1. The progressive-evidence v1 ablation is also complete: core plan + SpeedCoach passed 7/8 applicable checks but added an unsupported derived-distance deviation; context/environment passed 10/10; full mobile evidence passed 12/12; cross-condition consistency failed. The run cost US$0.298604 and is preserved unchanged. A versioned v2 candidate now marks reconstructed segment distance as insufficient for prescribed-distance completion, reinforces that boundary in the prompt, and rejects conflicting deviations in the verifier. Its three-condition dry-run is committed but no paid v2 result exists. The primary metric, first schema versions, 16-case registry, two public fixtures, permanent TDD policy, compact baseline bundle, comparable prompts, Python/uv evaluation runtime, GPT-5.6 Terra medium configuration, four deterministic investigation tools, bounded single-agent loop, verifier, observable trajectory format, monotonic case/run runtime instrumentation, coach-facing React/Vinext interface, local task-level product service, process-local typed source intake, deterministic SpeedCoach/WAKE-mobile normalization, progressive plan+SpeedCoach compact-summary preparation with optional mobile/environment/context enhancers, explicit prepared-bundle live execution, a generic analysis-to-review adapter, three frozen ground-truth-free evidence-ablation inputs, an all-condition runner, and a condition-aware capability reporter are accepted. Replay is the default; all live product execution requires explicit authorization and `OPENAI_API_KEY`. Only the exact five-source public case-002 upload bundle can reuse its replay. The results do not establish human-coach superiority or improved athletic performance. Checkpoint and memory persistence, database, authentication, club tenancy, and durable uploads remain unimplemented. Record new stable choices in `docs/DECISIONS.md`.
