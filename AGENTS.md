...
The project is in model-execution and grader development. The primary metric, first schema versions, 16-case registry, two public fixtures, permanent TDD policy, compact baseline bundle, comparable baseline/agent prompts, Python/uv evaluation runtime, GPT-5.6 Terra medium configuration, four deterministic investigation tools, bounded single-agent loop, verifier, and observable trajectory format are accepted. No paid model result, grader, database, persistent memory, UI framework, or product deployment target has been accepted yet. Record those choices in `docs/DECISIONS.md` before treating them as stable.
