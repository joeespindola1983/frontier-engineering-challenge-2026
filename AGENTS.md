...
The first controlled model comparison is complete. On the two implemented cases, the bounded WAKE agent scored 63.34/100 versus 38.86/100 for the direct-call baseline using frozen grader v1.1. The progressive-evidence v1 ablation is preserved unchanged: core plan + SpeedCoach passed 7/8 applicable checks but added an unsupported derived-distance deviation; context/environment passed 10/10; full mobile evidence passed 12/12; cross-condition consistency failed at US$0.298604. The versioned v2 workflow marks reconstructed segment distance as insufficient for prescribed-distance completion, reinforces that boundary in the prompt, and rejects conflicting deviations in the verifier. Its official repeat passed core 8/8, context/environment 10/10, full 12/12, and cross-condition consistency at US$0.358676. The primary metric, first schema versions, 16-case registry, two public fixtures, permanent TDD policy, compact baseline bundle, comparable prompts, Python/uv evaluation runtime, GPT-5.6 Terra medium configuration, four deterministic investigation tools, bounded single-agent loop, verifier, observable trajectory format, monotonic case/run runtime instrumentation, coach-facing React/Vinext interface, local task-level product service, process-local typed source intake, deterministic SpeedCoach/WAKE-mobile normalization, progressive plan+SpeedCoach compact-summary preparation with optional mobile/environment/context enhancers, explicit prepared-bundle live execution, generic analysis-to-review/checkpoint/briefing/memory transitions, per-execution cost authorization and process-local cost observability, three frozen ground-truth-free evidence-ablation inputs, an all-condition runner, and a condition-aware capability reporter are accepted. Replay is the default; all live product execution requires explicit mode, positive finite cost authorization, and `OPENAI_API_KEY`. The authorization is a start gate, not a provider cap. Only the exact five-source public case-002 upload bundle can reuse its replay. The results do not establish human-coach superiority, broad generalization, improved athletic performance, or durable billing control. Workflow state remains process-local; database, authentication, club tenancy, durable uploads, persistent checkpoint/memory storage, and durable accounting remain unimplemented. Record new stable choices in `docs/DECISIONS.md`.
