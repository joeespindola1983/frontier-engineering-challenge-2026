...
The first web slice follows the coach workflow: session inbox, evidence intake, review, one focused human checkpoint, verified briefing, and approval-gated goal memory. The hackathon build now adds a visually separate read-only Evaluation destination generated from the official ten-case artifacts; it contains no ground truth, raw evidence, or execution controls and makes no model call. The Sessions destination also contains a separate two-week **real-informed synthetic** club pulse with ten named crews across 2x, 4x, and 8x, sixteen fictional athletes, physical-boat identity, recurring lineup snapshots, crew-unavailable events, alternative solo/ergometer training, and athlete participation gaps. Its structures, workout patterns, plausible ranges, and failure modes were modeled from supplied real coach prescriptions, SpeedCoach CSVs, WAKE mobile export structures, Concept2 PM5 references, and first-hand club context. Its identities, exact history, lineups, outcomes, aggregates, and boat names are fictional and are not statistically representative. A cost-free period analyzer screens all 52 recorded activities, including fourteen individual Concept2 records, derives four session findings from compact observations rather than prefilled outcomes, and routes ten attention signals: two bounded agent candidates and eight human/source dependencies. Both complete candidate bundles passed preflight and were then owner-authorized individually. Their verified outputs isolate `work-02` at 18 SPM and `recovery-02` at 247 seconds; combined cost was US$0.194118 for 60,094 tokens. The interface records 35 water compact summaries, `2/2` completed investigations, eight non-model routes, no longitudinal synthesis, and athlete Training Days that keep water and indoor volumes separate. Crew and athlete drill-downs make those relationships visible without claiming that two synthetic sessions represent a real club or a longitudinal conclusion. The hosted version remains a clearly labeled synthetic replay of committed case 002. A local task-level HTTP service connects the same interface contract to replay or explicitly enabled live execution. Its tested process-local intake accepts five independently validated source types and returns only metadata to the browser. Either an athlete or coach may contribute the files; uploader identity is recorded separately from source origin and authority, so a coach can forward an athlete's SpeedCoach file and an athlete can forward a coach-authored plan without changing what each source can prove. SpeedCoach vendor, WAKE mobile sensor, and canonical telemetry CSVs are normalized deterministically with versioned provenance and quality flags. Progressive preparation requires only plan + SpeedCoach; mobile, environment, and context are independent enhancers whose absence becomes an explicit evidence gap. Source coverage is returned as core/enhancer and present/absent, and execution receives only the files actually supplied. A live-only endpoint executes that summary through the bounded agent with temporary normalized files and same-process idempotence. It also registers a process-local investigation so the page can continue the selected bundle through its own human confirmation, generic briefing, and explicit memory approval. New live execution requires a positive finite US$0.20 operational cost authorization; this is a start gate rather than a provider cap. The response and review expose trajectory-derived usage, runtime, approximate cost, and overrun status, while a process-local ledger counts each new execution once. Source-based replay remains restricted to the exact five-source public-bundle match. Durable storage, authentication, verified roles, tenancy, and accounting remain unimplemented.

The supplied visual prototype was retained as a direction, but repository evidence changed three important details: wind is a time-aligned association rather than a causal explanation; only one reviewed session appears in memory; and coach confirmation is stored as human context without rewriting telemetry. A submission-only Evaluation destination exposes the consolidated official result and ten public per-case score reports. Benchmark fixture controls, raw outputs, ground truth, and trajectories remain outside the coach workflow and browser controls.
...
than only relational frontend records. Fifty-two independent real-informed
synthetic activity directories preserve per-session source hashes. The no-cost
verifier validates and reconstructs all 52 records, compares 51 against a plan,
...
evidence, seventeen sessions are reconstructed alternatives, one needs a plan,
and one needs athlete context. Fourteen individual indoor records now pass a
deterministic adapter for declared-provenance Concept2 PM5 transcriptions and
cover fixed-distance, fixed-time, and interval shapes. Every PM5 result belongs
to one athlete. The athlete drill-down groups plan-confirmed pre-water,
post-water, alternative, and standalone indoor work into Training Days while
keeping water and indoor volume separate. The adapter was grounded by a
