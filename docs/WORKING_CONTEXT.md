...
The first web slice follows the coach workflow: session inbox, evidence intake, review, one focused human checkpoint, verified briefing, and approval-gated goal memory. The hackathon build also exposes a read-only Evaluation destination generated from the official ten-case artifacts. Sessions contains a separate two-week **real-informed synthetic** club pulse with ten named crews across 2x, 4x, and 8x, sixteen fictional athletes, physical-boat identity, recurring lineup snapshots, alternative water/ergometer training, and participation gaps. A cost-free analyzer screens all 52 activities, including fourteen individual Concept2 records, and routes ten attention signals. Two complete candidates were owner-authorized and verified for US$0.194118 combined. The completed longitudinal pilot then compared direct GPT and bounded WAKE on one athlete and one club scope. All four reports passed verification for US$0.110426; both workflows passed the same post-run capability checks, so no quality gain or post-hoc score is claimed, although WAKE used fewer tokens and cost 29.01% less. A second public post-regatta package adds 50 fictional activities for the same athletes and crews and demonstrates six deterministic period-comparison outcomes: comparable faster, comparable slower, stable, weather-confounded, participation review, and insufficient evidence. One compact WAKE synthesis over the combined 102-activity club screen is verified and saved with `store: false`; it cost US$0.037384, used 6,322 tokens, called all four deterministic tools, and reopens at US$0.00. Crew and athlete drill-downs, Training Days, Competition Review, the second-period load, and the saved club memory show relational continuity without claiming real athletic improvement or causation. The hosted version remains a clearly labeled synthetic replay. A local task-level HTTP service connects the same interface contract to replay or explicitly enabled live execution. Its tested process-local intake accepts five independently validated source types; either an athlete or coach may contribute files, while uploader identity remains separate from source origin and authority. Progressive preparation requires only plan + SpeedCoach; mobile, environment, and context are independent enhancers. New live execution requires a positive finite operational start authorization that is not a provider cap. Durable storage, authentication, verified roles, tenancy, and production accounting remain unimplemented.
...
completed two-case experiment rather than an undefined future call: one athlete
briefing and one club-priority briefing, each compared as direct baseline versus
bounded WAKE. The four verified reports cost US$0.110426; both workflows passed
the same post-run capability checks, so the honest conclusion is
`NO_DEMONSTRATED_QUALITY_GAIN`. A separate one-request preflight then covered the
combined 102-activity pre/post-regatta club screen. After explicit authorization,
the bounded WAKE run called all four tools and produced one verified local report
for US$0.037384 and 6,322 tokens. It preserves three narrow comparable indoor
observations, abstains from a club performance trend, ranks three coach-review
priorities, asks four human questions, and reopens without another model call.
