...
The first web slice now follows the coach workflow rather than the hackathon evaluation workflow: session inbox, evidence intake, review, one equipment checkpoint, verified briefing, and approval-gated goal memory. The hosted version remains a clearly labeled synthetic replay of committed case 002. A local task-level HTTP service now connects the same interface contract to replay or explicitly enabled case-level live execution. Its tested process-local intake accepts five independently validated source types and returns only metadata to the browser. Source-based replay is restricted to an exact byte match with the public bundle; normalization and live analysis of a different uploaded bundle, durable storage, authentication, and tenancy remain unimplemented.
