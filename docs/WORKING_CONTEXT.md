...
The first web slice now follows the coach workflow rather than the hackathon evaluation workflow: session inbox, evidence intake, review, one equipment checkpoint, verified briefing, and approval-gated goal memory. The hosted version remains a clearly labeled synthetic replay of committed case 002. A local task-level HTTP service now connects the same interface contract to replay or explicitly enabled case-level live execution. Its tested process-local intake accepts five independently validated source types and returns only metadata to the browser. SpeedCoach vendor, WAKE mobile sensor, and canonical telemetry CSVs are normalized deterministically with versioned provenance and quality flags. A new five-source preparation endpoint now assembles a schema-validated compact summary with route, clock, distance, environmental, and missing-evidence findings without calling a model. Source-based replay remains restricted to an exact public-bundle match. Live analysis of a prepared different bundle, durable storage, authentication, and tenancy remain unimplemented.
