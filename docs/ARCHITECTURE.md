...
A separate `wake.club_period_analysis.v1` screen now sits between the relational demo club and any future paid longitudinal execution. It deterministically screens every recorded activity, derives attention from compact observations with evidence references, and routes human/source dependencies away from the model queue. This is orchestration policy, not a longitudinal model result: the current club dataset has 35 compact evidence summaries and two complete public synthetic source bundles. The public verifier proves their hashes, privacy invariants, plan schemas, and expected v2 reconstruction before the two candidate investigations become `READY_FOR_AUTHORIZATION`. They remain `0/2` completed and synthesis remains `NOT_EXECUTED` until an explicit paid run is performed.
