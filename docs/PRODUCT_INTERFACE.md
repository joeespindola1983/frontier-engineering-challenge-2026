...
`buildClubPeriodAnalysis` screens all 52 recorded activities and all 38 planned outings without a model call. It derives SPM, recovery, plan-link, and athlete-context findings from compact observations with evidence references; combines them with crew-unavailable and participation-gap signals; and routes each item to agent investigation, an athlete question, human context, or a source request. The current result contains ten attention signals, but only two were session-level model candidates. Both complete bundles passed deterministic preflight, received explicit authorization, and produced verified v2 outputs for US$0.194118 combined. The separate four-report longitudinal pilot cost US$0.110426 and did not demonstrate a quality advantage over its direct baseline. `buildPostRegattaComparison` is deliberately deterministic: loading the committed second period costs US$0.00 and shows how accumulated evidence can support or refuse different comparisons without treating a faster/slower value as a causal athletic conclusion.

The separate public batch gives every displayed activity an isolated source directory and hash manifest. Its verifier reports 52 data-validated and reconstructed records, 51 plan comparisons, two agent-verified sessions, and zero human approvals. Thirty-one water sessions have no material signal in available evidence, seventeen sessions are reconstructed alternatives, one needs a plan, and one needs athlete context. Fourteen individual indoor records use deterministic synthetic Concept2 PM5 transcription-format records; a separate minimized real-reference packet contains human-confirmed values. The interface explicitly states that photo OCR is not implemented and exposes validation levels rather than collapsing them into one “homologated” state.

The Competition Review remains part of the same deterministic fictional club, not an import of the supplied real athlete names. It uses official documents only to inform structure: repeated race numbers, multiple entries per club, public rank and time, displayed-time ties, non-classified entries, and category-based distances. The earlier same-federation programme is a `REFERENCE_ONLY` source for distance and cannot prove the exact distance used by a separate stage. Event identity is therefore composite, and every derived pace retains the distance evidence reference plus `NOT_DIRECTLY_OBSERVED` stage confirmation. The report preserves the published order when displayed times tie, asks for human context when a crew is not classified, and labels pre-race shared outings as descriptive context rather than a cause or crew-selection recommendation. An in-content Sessions action keeps this view reachable when compact navigation hides the primary links.
...
- attach a real athlete identity to fictional training or competition history;
- infer competition distance from boat class when the category rule or direct programme is unavailable;
- recompute official rank from displayed times or claim that training caused a race result.
