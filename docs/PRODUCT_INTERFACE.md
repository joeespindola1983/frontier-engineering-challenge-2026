...
The separate public batch gives every displayed activity an isolated source directory and hash manifest. Its verifier reports 40 data-validated and reconstructed records, 39 plan comparisons, two agent-verified sessions, and zero human approvals. Thirty-one water sessions have no material signal in available evidence, five sessions are reconstructed alternatives, one needs a plan, and one needs athlete context. The two indoor alternatives use deterministic synthetic Concept2 PM5 transcription-format records; a separate minimized real-reference packet contains human-confirmed values. The interface explicitly states that photo OCR is not implemented and exposes validation levels rather than collapsing them into one “homologated” state.
