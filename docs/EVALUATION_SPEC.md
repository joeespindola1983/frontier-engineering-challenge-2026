...
The frozen version 1 artifacts are `prompts/baseline-v1.md` and `evaluation/baseline-inputs/v1/`. They remain byte-stable and contain the two cases used by the first controlled comparison. The expanded `evaluation/baseline-inputs/v2/` bundle preserves those two exact summary entries and adds ground-truth-free summaries for cases 003-010. It is generated by `scripts/build_baseline_inputs.py` and checked alongside v1 for provenance hashes and evaluator-answer leakage by `scripts/verify_baseline_inputs.py`. The v2 bundle is input readiness, not a scored result; its new cases remain outside the denominator until grader support and required outputs are committed.
...
The committed expanded preflight contains ten baseline and ten agent request
objects with `api_called: false`; paid outputs do not yet exist.
