  ...
  return (
    <section className="club-overview" aria-labelledby="club-pulse-title">
      ...
      <section className="longitudinal-pilot-card" aria-labelledby="longitudinal-pilot-card-title">
        ...
        <div className="longitudinal-pilot-summary"><span><strong>4 saved reports</strong><small>reopen at US$0.00</small></span><span><strong>US${pilot.observed.total_cost_usd.toFixed(6)}</strong><small>observed total cost</small></span><span><strong>No demonstrated quality gain</strong><small>same capability checks passed</small></span></div>
        <button className="button button-primary" onClick={onOpenLongitudinalPilot} type="button">Inspect saved comparison</button>
      </section>
...
function LongitudinalPilotScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <main className="page longitudinal-pilot-page">
      ...
      <section className="longitudinal-case-section" aria-labelledby="longitudinal-case-title"><div className="section-heading"><div><div className="kicker">Saved comparison cases</div><h2 id="longitudinal-case-title">Athlete briefing + Club briefing</h2></div><p>Selective synthesis follows deterministic screening.</p></div><div className="longitudinal-case-grid">{pilot.cases.map((item) => <article key={item.pilot_id}><header><span>{item.scope_type}</span><strong>{item.title}</strong><small>{item.subject}</small></header><div><h3>Why GPT is used</h3><p>{item.why_model_is_used}</p><h3>Observed result</h3><p>{item.result_summary}</p></div><dl><div><dt>Deterministic coverage</dt><dd>{item.deterministic_coverage}</dd></div><div><dt>Comparison boundary</dt><dd>{item.comparison_status}</dd></div></dl><footer>{item.required_tools.map((tool) => <span key={tool}>{tool}</span>)}</footer></article>)}</div></section>
      <section className="longitudinal-method" aria-labelledby="longitudinal-method-title"><div><div className="kicker">Controlled comparison</div><h2 id="longitudinal-method-title">What we learned</h2><p>Both workflows covered the required evidence, preserved human review, and abstained from performance trends. WAKE used four bounded tools per scope and fewer tokens, but did not demonstrate a quality advantage on these two cases.</p></div><ol><li><span>01</span><div><strong>Direct baseline</strong><p>{pilot.observed.baseline_tokens.toLocaleString('en-US')} tokens · US${pilot.observed.baseline_cost_usd.toFixed(5)} · all checks passed.</p></div></li><li><span>02</span><div><strong>WAKE bounded agent</strong><p>{pilot.observed.wake_tokens.toLocaleString('en-US')} tokens · US${pilot.observed.wake_cost_usd.toFixed(6)} · {pilot.observed.wake_tool_events} tool events · all checks passed.</p></div></li><li><span>03</span><div><strong>Honest conclusion</strong><p>No quality score is presented after seeing the outputs. The result supports lower resource use here, not superior longitudinal reasoning.</p></div></li></ol></section>
