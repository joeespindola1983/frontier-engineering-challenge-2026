...
function ClubOverview({ onOpenCrew, onOpenAthlete }: { onOpenCrew: (crewId: string) => void; onOpenAthlete: (athleteId: string) => void }) {
  ...
  return (
    <section className="club-overview" aria-labelledby="club-pulse-title">
      ...
      <section className="club-intelligence-status" aria-label="Club intelligence status"><div><span>Records screened</span><strong>{intelligence.coverage.activities_scanned}/{demoClub.activities.length}</strong><small>all recorded activities</small></div><div><span>Deep investigations</span><strong>{intelligence.deep_investigations.completed}/{intelligence.deep_investigations.queued}</strong><small>{intelligence.coverage.complete_source_bundles} complete bundles · authorization pending</small></div><div><span>Human or source routes</span><strong>{intelligence.routing.HUMAN_CONTEXT + intelligence.routing.ATHLETE_QUESTION + intelligence.routing.SOURCE_REQUEST}</strong><small>no paid call required yet</small></div><div><span>Planning projection</span><strong>US${intelligence.cost_forecast.planning_projection_usd.toFixed(2)}</strong><small>{intelligence.cost_forecast.paid_executions} future paid executions</small></div></section>
      <div className="club-intelligence-boundary" role="note"><strong>Deep agent analysis has not run yet.</strong><span>Every record has passed deterministic screening. Two complete bundles passed deterministic preflight and are ready for explicitly authorized bounded investigation; human and missing-source questions stay outside the paid queue. The longitudinal synthesis remains pending.</span></div>
