...
The repository contains one difficult anonymized multi-device fixture, nine deterministic synthetic fixtures, a 16-case registry, versioned JSON Schemas, deterministic raw telemetry adapters, progressive two-to-five-source compact-summary preparation, an explicit prepared-bundle runner, a generic coach-review adapter, a local persistent session inbox, comparable baseline and agentic OpenAI Responses API runners, offline graders, the [historical two-case comparison](evaluation/runs/comparison-v1-20260829/README.md), the [official ten-case comparison](evaluation/runs/expanded-evaluation-v2/official-20260830/README.md), and a coach-facing replay with a separate no-cost Evaluation view. The completed two-case longitudinal pilot preserves four verified reports for an athlete briefing and club-priority briefing. WAKE did not demonstrate a quality gain over the direct baseline on those two cases, but used fewer tokens and cost 29.01% less; the four reports cost US$0.110426 combined and reopen at US$0.00. A second public [post-regatta package](data/demo-club-post-regatta/v1/README.md) adds 50 real-informed synthetic activities and six deterministic comparison scenarios. One bounded WAKE synthesis over the combined 102-activity club screen is now verified and saved: 6,322 tokens, 22.507 seconds, and US$0.037384, with zero-cost reopening. Registry v1 preserves the historical two-case denominator; registry v2 promotes cases 001-010 only after their paid outputs, trajectories, and offline grading were committed.
...
The safe default rebuilds one bounded WAKE request over the deterministic screen of 102
real-informed synthetic activities, 16 athletes, 10 crews, and six period
comparison routes without calling the API. The preserved verified report lives
under `evaluation/runs/post-regatta-memory-v1-20260830/`; it used `store: false`,
cost US$0.037384, and can be reopened at US$0.00. Any new live execution remains separate and
