...
The club layer now performs a cost-free deterministic screen over all 40 recorded activities and the 38 planned crew outings. Four session findings are derived from compact observations rather than stored as prewritten outcomes: low work-interval SPM, excessive recovery, a missing plan link, and missing athlete effort context. Together with crew disruptions and participation gaps, the screen produces ten evidence-referenced attention signals. Two are candidates for bounded agent investigation; eight are routed to a person or missing source without a paid call. Both numeric candidates now have complete, public, real-informed synthetic plan + SpeedCoach + context bundles under `data/demo-club-evidence/`. Their hashes, privacy boundary, schemas, and expected deterministic reconstructions pass the public preflight. The interface therefore reports `0/2` paid investigations with `2` complete bundles ready for explicit authorization; no longitudinal model output is claimed.
